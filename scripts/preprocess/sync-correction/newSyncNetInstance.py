...
class SyncNetInstance(torch.nn.Module):
    ...
    def separate_frames(self, opt, videofile):
        ...
        command = ("ffmpeg -loglevel error -y -i %s -threads 1 -f image2 %s" % (video_path, os.path.join(save_dir, '%06d.png'))) 
    ...
    def evaluate(self, opt, videofile):
        ...
        if (float(len(audio))/sample_rate) != (float(len(images))/25) :
            if self.verbose:    
                print("WARNING: Audio (%.4fs) and video (%.4fs) lengths are different."%(float(len(audio))/sample_rate, float(len(images))/25))
